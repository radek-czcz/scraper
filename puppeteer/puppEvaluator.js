...
async function main() {
   ...
   //const page = ppage.getPage().then(res => res);
   /*let page = [ppage.getExistingPage(), ppage.getPage()];
   page = page[0] ? page[0] : page[1];*/
   let page = ppage.getPage();
   return page.then(async res => {
      const dataExtract = await /*ppage.getPage()*/res.evaluate(() => {
